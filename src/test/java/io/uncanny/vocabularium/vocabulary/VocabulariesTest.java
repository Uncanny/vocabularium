...
import io.uncanny.vocabularium.vocabulary.Vocabularies;

public class VocabulariesTest {

	private static final Path ROOT=Paths.get("src","test","resources","failures");
	private static final URI BASE=URI.create("http://www.smartdeveloperhub.org/vocabulary/");

	@Test
	public void validatesVersioningHierarchy() throws Exception {
		final Result<Catalog> result = Catalogs.loadFrom(ROOT,BASE);
		final Vocabularies vocabularies = Vocabularies.create(result.get());
		System.out.println(vocabularies);
		for(final String failure:vocabularies.validate()) {
